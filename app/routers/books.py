...
def get_book_info(book_id: int, db: Session = Depends(dependencies.get_db)):
...
def download_book_by_id(book_id: int, db: Session = Depends(dependencies.get_db)):
    book = crud.get_book_by_id(db, book_id=book_id)
    if book is None:
        raise HTTPException(status_code=404, detail="Book not found")
...
def add_book_info(book: schemas.BookCreate, db: Session = Depends(dependencies.get_db)):
